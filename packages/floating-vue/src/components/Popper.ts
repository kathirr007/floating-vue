  ...
  defineComponent({
    ...
    computed: {
      ...
      hasPopperShowTriggerHover() {
        return (
          this.popperTriggers?.includes('hover') ||
    ...
    watch: {
      ...
      ...['triggers', 'positioningDisabled'].reduce((acc, prop) => {
    ...
    methods: {
      ...
      async $_applyShowEffect() {
        ...
        if (this.computeTransformOrigin) {
          ...
          const popperWrapper =
            this.$_popperNode.querySelector('.v-popper__wrapper');
          const parentBounds = popperWrapper.parentNode.getBoundingClientRect();
          const x =
            bounds.x +
            bounds.width / 2 -
            (parentBounds.left + popperWrapper.offsetLeft);
          const y =
            bounds.y +
            bounds.height / 2 -
            (parentBounds.top + popperWrapper.offsetTop);
      ...
      $_isAimingPopper() {
        const referenceBounds: DOMRect =
          this.$_referenceNode.getBoundingClientRect();
        if (
          ...
          const popperBounds: DOMRect =
            this.$_popperNode.getBoundingClientRect();
          ...
          const newVectorLength =
            distance + popperBounds.width + popperBounds.height;
...
function handleGlobalClose(event, touch = false) {
  ...
  for (let i = shownPoppers.length - 1; i >= 0; i--) {
    const popper = shownPoppers[i];
    try {
      const contains = (popper.$_containsGlobalTarget = isContainingEventTarget(
        popper,
        event
      ));
